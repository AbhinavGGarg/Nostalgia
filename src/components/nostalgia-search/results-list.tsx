...
export function ResultsList({ items, savedIds, onToggleSave }: ResultsListProps) {
  return (
    <section className="ns16-results">
      {items.map((item, index) => {
        if (item.kind === "real") {
          return (
            <ResultItem key={item.id} item={item} index={index} isSaved={savedIds.has(item.id)} onToggleSave={onToggleSave} />
          );
        }
